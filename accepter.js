...
let render = (donorData, donorBloodType) =>{
    ...
    //Getting donor's name and email and rendering
        ...
        let donorContact = document.createElement("TD");
        let donorEmail = document.createElement("TD");
        let donorAddress = document.createElement("TD");
        let donorGender = document.createElement("TD");
// console.log(donorName);
        database.ref('/userObj/' + donorData.userID).once('value').then(function(snapshot) {

            let donorInfo = snapshot.val();
            console.log(donorInfo);

            //Adding donor name
            let donorNameText = document.createTextNode(donorInfo.Sname);
            donorName.appendChild(donorNameText);
            // console.log(donorNameText);

            //Adding donor Contact #
            let donorContactText = document.createTextNode(donorInfo.Contact);
            donorContact.appendChild(donorContactText);
            
            //Adding donor email
            let donorEmailText = document.createTextNode(donorInfo.Semail);
            donorEmail.appendChild(donorEmailText);

             //Adding donor Gender
            let donorGenderText = document.createTextNode(donorInfo.gender);
            donorGender.appendChild(donorGenderText);
            
            //Adding donor Address
            let donorAddressText = document.createTextNode(donorInfo.Scityname);
            donorAddress.appendChild(donorAddressText);
    ...
    //Appending to TableBody
        ...
        donorListRow.appendChild(donorContact);
        donorListRow.appendChild(donorEmail);
        donorListRow.appendChild(donorGender);
        donorListRow.appendChild(donorAddress);
